...
export async function deleteUser(env, account, targetId) {
  ...
  // push_subscriptions/known_matches' ON DELETE CASCADE foreign keys
  // actually being enforced — SQLite (and by extension D1) only enforces
  // FK constraints when foreign_keys is turned on for the connection,
  // which nothing in this codebase does, so this can't assume it's
  // active. Everything trade_id-scoped goes first since it references
  ...
  // every message/report/rating they were involved in, since a rating's
  // rater and rated account are always the trade's two participants, same
  // as messages/reports only ever happening on your own trade.
  await env.DB.batch([
