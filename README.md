...
**Known gap:** a trade's card snapshot (see **Admin** below) doesn't currently carry the photo forward — it copies the card's other fields at propose time but not `photo_key`, so an admin reviewing a trade dispute sees the card details but not the verification photo, even though the same authenticated-photo infrastructure could serve it. Worth adding if photo evidence turns out to matter for actual disputes.
