...
- `src/components/` — `Onboarding` (signup + login, toggled), `BottomNav`, `HomeView`, `CollectionView`, `CardPicker`, `CardChip`, `CardMeta` (set/number/price, shared by `CardChip` and `CardPicker`), `MatchesView`, `TradesView`, `ValueDisparityModal`, `ProfileView`, `AvatarPicker`, `AvatarIcon`.
